...
- **Опциональный honeypot-сенсор** — low-interaction TCP-слушатель (например, на SSH-порту): доступа **не** даёт, но логирует, кто подключился, и баннер клиента, эмитя `intrusion`-события в то же ядро (store/алерты/страница статуса). Второй сенсор sentinel — по-прежнему только stdlib; всплеск новых источников агрегируется в один алерт, а не спамит.
...
### Honeypot (опциональный сенсор вторжений)

| Поле | Тип | По умолчанию | Назначение |
|---|---|---|---|
| `honeypot.enabled` | bool | `false` | Включить low-interaction TCP-ханипот. |
| `honeypot.port` | int | `2222` | Порт прослушивания (≥1024, если не под root). |
| `honeypot.host` | string | `""` | Адрес привязки (пусто = все интерфейсы). |
| `honeypot.banner` | string | `""` | Баннер, отправляемый при подключении (например `SSH-2.0-OpenSSH_8.9`), чтобы вытянуть баннер клиента. |

Слушатель биндит порт только в режимах `run` / `serve` (никогда при `check` / `status`). Доступа не даёт — лишь пишет каждое подключение (событие INFO) и на **первое** появление IP в окне переалерта шлёт **WARNING**; всплеск многих новых источников агрегируется в одну сводку, чтобы скан не спамил. Наведи на публичный хост, чтобы ловить реальный трафик.
...
### `sentinel serve [-c config.json] [--port 8787]`

Запускает постоянный цикл мониторинга **и** отдаёт страницу статуса по HTTP в одном процессе — без обратного прокси и статического хостинга. `tick()` крутится в фоновом daemon-потоке каждые `interval_seconds`, а в основном потоке работает stdlib-сервер `http.server`, который на каждый запрос рендерит **свежий** снапшот, поэтому страница всегда актуальна. Ctrl+C аккуратно останавливает оба потока.

Маршруты:

| Путь | Ответ |
|------|-------|
| `/` | HTML-страница статуса (тёмная тема). |
| `/status.json` | Полный снапшот в JSON. |
| `/health` | Тот же JSON-снапшот (удобно как health-эндпоинт / проба аптайма). |
| *всё остальное* | `404 Not Found`. |

`--port` по умолчанию `8787`; `--host` по умолчанию — все интерфейсы (укажите `127.0.0.1`, чтобы оставить локально). Всё — только на стандартной библиотеке.
...
Страница перезаписывается в двух случаях: командой `status` и автоматически на каждом тике/проверке, если в конфиге задан `status_page`. Кроме того, команда `serve` отдаёт эту же страницу (и JSON) по HTTP на лету — встроенным stdlib-сервером, без внешнего хостинга.
...
Сенсоры — это плагинный шов. Сенсор наследует базовый класс `Sensor` (`sentinel/sensors/__init__.py`) и эмитит объекты `Event` в общее ядро; ядро сохраняет, алертит и рендерит их единообразно. Сегодня поставляются **два** сенсора — и они доказывают, что шов работает для обоих стилей:

- **`uptime`** — *поллинговый*: `poll()` гоняет проверки и возвращает новые события каждый тик.
- **`honeypot`** — *listener-based*: фоновый сокет-сервер ловит подключения и буферизует их; `poll()` дренит буфер в `intrusion`-события. Всё попадает в то же SQLite-хранилище, те же Telegram-алерты и ту же страницу статуса.

Добавить третий сенсор — это написать один `poll()`, ядро не трогается. Поле `Event.kind` покрывает их все (`"state_change"`, `"cert_expiring"`, `"honeypot_hit"`, `"intrusion"`).
